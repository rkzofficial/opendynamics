...
// Get Dynamics client for a specific user (used for public shared access)
export async function getDynamicsClientForUser(userId: string): Promise<DynamicsClientResult> {
  const config = useRuntimeConfig()
  const convex = getConvexClient()

  // Get user's tokens
  const tokens = await convex.query(api.tokens.getForUser, { userId: userId as Id<'users'> })

  if (!tokens) {
    throw createError({
      statusCode: 401,
      message: 'User not connected to Dynamics CRM',
    })
  }
  ...
  if (expiresAt < Date.now() + 5 * 60 * 1000) {
    ...
    try {
      ...
      throw createError({
        statusCode: 401,
        message: 'Dynamics session expired for share owner',
      })
    }
  }
...
/**
 * Get Dynamics client with a pre-authenticated user.
 * Use this with cachedAuthHandler where the user is already authenticated.
 * 
 * @param user - Pre-authenticated user from cachedAuthHandler
 * @param targetUserId - Optional target user ID (admin only)
 */
export async function getDynamicsClientWithUser(user: AuthUser, targetUserId?: string): Promise<DynamicsClientResult> {
  const config = useRuntimeConfig()
  const convex = getConvexClient()
